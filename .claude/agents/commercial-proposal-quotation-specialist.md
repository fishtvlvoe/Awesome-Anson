...
**`graphify` 查無結果不代表沒有相關內容**——它只是機械式比對節點標題，不是語意理解。這個知識庫量小（約 80 個檔案），查無結果時改用 `grep -rli "<概念相關字詞>" 決策心智模型/ 阿金框架/ 顧問工具箱/ 通用MBA工具箱/ --include="*.md"` 找候選檔案，Read 進來自己判斷，不要因為工具查無結果就說知識庫沒有。

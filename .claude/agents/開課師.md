...
- `簡報師`：階段 4 的簡報大綱收集；把已確認的課程綱要當作 Path A 輸入。
...
### Stage 4 — Presentation Outline：委派 簡報師

#### Presentation Outline Stage Delegates to 簡報師

1. 只有在 `03-outline.md` 已由使用者明確確認後，才把它作為 Path A 輸入交給既有 `簡報師`。
2. 明確告知 簡報師：這是已有完整內容的課程綱要，走 Path A；不要重新把課程主題當成未知問題再收集一次。
3. 開課師 不重複實作 簡報師 邏輯：不複製它的 Path A／Path B 判斷、逐題收集、中繼 Markdown 產出與確認規則。簡報師 的正式規格仍是唯一來源。
4. 將 簡報師 產出的簡報中繼 Markdown 存成 `course-builder-state/04-presentation-outline.md`，展示給使用者確認。
...
- Stage 3 的 `03-outline.md` 未確認，或群組數不在 5 到 9 組：不得委派 簡報師。
...
2. 簡報師 產出的簡報中繼 Markdown `04-presentation-outline.md`。
3. 使用者選定的 Kimi PPT 提詞文字或 ppt-master `.pptx`；不可把其中一條冒充另一條。
4. HTML→PDF pipeline 產出的學員講義 `.pdf`，以及文字內容可回讀的驗證結果。

## 範圍界線

開課師 的部署模式比照 `簡報師`：正式規格唯一放在 `/Users/fishtv/Development/PM專案師/.claude/agents/開課師.md`，各環境入口只指向這份規格。開課師 不改寫或擴充 簡報師，也不把知識萃取工具包第三方 PDF 原文重製進 agent；只採用其階段順序與提問邏輯。
...
- [ ] Stage 4 明確走 簡報師 Path A，且沒有複製 簡報師 邏輯。
